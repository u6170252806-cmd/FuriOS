...
#ifdef FUROS_TCC
static int furos_ptr_sane(const void *p)
{
    uintptr_t v = (uintptr_t)p;
    if (v == 0)
        return 1;
    if ((v & (sizeof(uintptr_t) - 1)) != 0)
        return 0;
    if (v == ~(uintptr_t)0)
        return 0;
    /* FuriOS EL0 VA is intentionally small and contiguous. */
    if (v < 0x00001000u || v >= 0x01000000u)
        return 0;
    return 1;
}

static int furos_tokensym_ptr_ok(TokenSym *ts)
{
    return furos_ptr_sane(ts);
}

static int furos_sym_ptr_ok(Sym *s)
{
    return furos_ptr_sane(s);
}
#else
#define furos_tokensym_ptr_ok(ts) 1
#define furos_sym_ptr_ok(s) 1
#endif
...
ST_INLN Sym *define_find(int v)
{
    ...
    if ((unsigned)v >= (unsigned)(tok_ident - TOK_IDENT))
        return NULL;
    if (!furos_tokensym_ptr_ok(table_ident[v])) {
        table_ident[v] = NULL;
        return NULL;
    }
    if (!furos_sym_ptr_ok(table_ident[v]->sym_define)) {
        table_ident[v]->sym_define = NULL;
        return NULL;
    }
...
#else
        /* In compile modes we only need the macro predefs section; forcing
           __TCC_PP__ for this include skips the heavier builtin shim block,
           which has proven unstable under long-run FuriOS toolchain flows. */
        "#if __TINYC__\n"
        "# ifndef __TCC_PP__\n"
        "#  define __TCC_PP__ 1\n"
        "#  include <tccdefs.h>\n"
