...
{
    if ((type->t ^ sym->type.t) & VT_THREAD)
        tcc_error("thread-local mismatch for redefinition of '%s'",
                  get_tok_str(sym->v, NULL));
...
static int parse_btype(CType *type, AttributeDef *ad, int ignore_label)
{
    ...
    while(1) {
       ...
       storage:
        ...
        case TOK___THREAD2:
            if (t & VT_TYPEDEF)
                tcc_error("thread-local used with typedef");
            t |= VT_THREAD;
            next();
            break;
        default:
...
static int decl(int l)
{
    ...
    while (1) {
	    ...
	    ad = adbase;
            ...
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                if (type.t & VT_THREAD)
                    tcc_error("function cannot be thread-local");
		...
		if (l == VT_CMP) {
		    ...
		    tcc_error("declaration of void object");
                    ...
                    if ((type.t & VT_THREAD) && l != VT_CONST
                        && !(type.t & (VT_STATIC | VT_EXTERN)))
                        tcc_error("block-scope thread-local must be static or extern");
                    r = 0;
                    if ((type.t & VT_BTYPE) == VT_FUNC) {
