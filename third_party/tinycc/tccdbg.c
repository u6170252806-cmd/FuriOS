...
#ifdef FUROS_TCC
static int furos_dbg_ptr_sane(const void *p)
{
    ...
    if (v == ~(uintptr_t)0)
        return 0;
    if (v < 0x00001000u || v >= 0x0001000000000000ull)
        return 0;
    return 1;
}

static const char *furos_dbg_str_or_unknown(const char *s)
{
    if (!furos_dbg_ptr_sane(s))
        return "<invalid>";
    return s;
}
#endif
...
static void dwarf_string(Section *s, Section *dw, int sym, const char *str)
{
    TCCState *s1 = s->s1;
#ifdef FUROS_TCC
    str = furos_dbg_str_or_unknown(str);
#endif
    ...
    while (new_hash) {
#ifdef FUROS_TCC
        if (!furos_dbg_ptr_sane(new_hash)) {
            dw_hash[hash] = NULL;
            new_hash = NULL;
            break;
        }
...
#ifdef FUROS_TCC
    if (!furos_dbg_ptr_sane(file->filename)) {
...
static void tcc_debug_finish (TCCState *s1, struct _debug_info *cur)
{
    ...
    while (cur) {
        ...
        if (!furos_dbg_ptr_sane(cur))
            break;
        if (cur->n_sym < 0 || cur->n_sym > (1 << 20))
            break;
        if (cur->sym && !furos_dbg_ptr_sane(cur->sym))
            cur->sym = NULL, cur->n_sym = 0;
        if (next && !furos_dbg_ptr_sane(next))
            next = NULL;
        if (cur->child && !furos_dbg_ptr_sane(cur->child))
            cur->child = NULL;
#endif
