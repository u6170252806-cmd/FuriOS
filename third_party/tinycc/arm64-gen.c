...
static int arm64_sym_is_tls(Sym *sym)
{
    return sym && (sym->type.t & VT_THREAD);
}

static void arm64_sym_tls(int r, Sym *sym, unsigned long addend)
{
    greloca(cur_text_section, sym, ind, R_AARCH64_TLSIE_ADR_GOTTPREL_PAGE21, 0);
    o(0x90000000 | r);            // adrp xr, :gottprel:sym
    greloca(cur_text_section, sym, ind, R_AARCH64_TLSIE_LD64_GOTTPREL_LO12_NC, 0);
    o(0xf9400000 | r | (r << 5)); // ldr xr,[xr, #:gottprel_lo12:sym]
    if (r == 30) {
        /* Preserve x9 while using it as TP scratch when destination is x30. */
        o(0xf81f0fe0 | 9); // str x9, [sp, #-16]!
        o(0xd53bd040 | (uint32_t)9); // mrs x9,tpidr_el0
        o(0x8b000000 | r | (r << 5) | (uint32_t)9 << 16); // add x30, x30, x9
        o(0xf84107e0 | 9); // ldr x9, [sp], #16
    } else {
        o(0xd53bd040 | (uint32_t)30); // mrs x30,tpidr_el0
        o(0x8b000000 | r | (r << 5) | (uint32_t)30 << 16); // add xr, xr, x30
    }
    if (addend) {
        if (addend & 0xffful)
            o(0x91000000 | r | r << 5 | (addend & 0xfff) << 10);
...
ST_FUNC void load(int r, SValue *sv)
{
	...
	if (sv->sym)
            if (arm64_sym_is_tls(sv->sym))
                arm64_sym_tls(30, sv->sym,
    ...
    if (svr == (VT_CONST | VT_SYM)) {
        if (arm64_sym_is_tls(sv->sym))
            arm64_sym_tls(intr(r), sv->sym, svcul);
        else
